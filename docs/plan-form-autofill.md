...
> ~~Публичные ссылки — плейсхолдеры~~ — репозиторий создан под именем `itchernetski/formless` (public, 2026-08-08), все ссылки резолвятся, заменять нечего.

- [x] README (positioning: privacy-first, local, AI generation), скриншоты, GIF-демо — `README.md` (позиционирование, фичи, приватность, permissions-таблица, архитектура, roadmap); скриншоты генерируются `scripts/screenshots.mjs` → `docs/assets/` (headless Chromium + chrome-стаб + сид IndexedDB). ~~Оверлей и AI-панель — ручной захват~~ — сцены гоняют реальный content script против `scripts/fixtures/job-application.html`, ручного захвата больше не нужно; композиты для стора — `npm run store-shots` → `release/store/0{1..4}-*.png`. Остался ручным только GIF-демо (2026-08-08)
...
- [~] Сборка и публикация в Chrome Web Store (privacy policy обязательна) — **сборка готова**: `npm run package` → `release/formless-0.1.0-chrome.zip`; листинг, short/detailed description, permission justifications, data-disclosure ответы, single-purpose statement — `docs/store-listing.md`; пошаговая процедура публикации (аккаунт → поля дашборда → privacy-таб → rejection playbook → апдейты) — `docs/chrome-web-store.md`; privacy policy — `PRIVACY.md` + хостируемая `landing/privacy.html`. **Осталось только оформление в дашборде** — всё остальное закрыто (2026-08-08):
  - [x] permission `tabs` снят, манифест просит `storage` + `activeTab`; проверено в живом Chrome (§0.1)
  - [x] публичный URL политики — https://itchernetski.github.io/formless/privacy.html, деплоится `.github/workflows/pages.yml` из `landing/`
  - [x] 4 скриншота 1280×800 — `npm run store-shots` → `release/store/`, размер и непрозрачность проверяет сам скрипт (§3)
  - [x] support-контакт — `tchernetski@gmail.com`
  - [ ] **Завершить регистрацию dev-аккаунта** — Account settings в Developer Console: publisher name, contact email (подтвердить), privacy policy URL, **trader status** (влияет на распространение в ЕС — читать, а не прокликивать), 2FA, разовые $5. Заполнить ДО создания item, иначе форма item блокируется на пустом поле. Ссылка на конкретный экран настроек — в задаче сессии, не здесь: в ней UUID аккаунта, а репозиторий публичный. См. `docs/chrome-web-store.md` §1
  - [ ] Создать item, залить `release/formless-0.1.0-chrome.zip`, заполнить листинг из `docs/store-listing.md`, подать **Unlisted** (§2-§7)
...
- [ ] Alpha-прогон в Telegram/среди знакомых, сбор фидбека — ~~блокировано ручным прогоном и публикацией репо~~, оба сделаны (2026-08-08). Ждёт ссылку на **Unlisted**-листинг: раздавать её конвертит сильно лучше, чем «склонируй и собери»

**Выход фазы:** установка из стора, репозиторий публичен, PH-лонч готов. — ⏳ репозиторий **опубликован** (github.com/itchernetski/formless, public, CI зелёный, Pages живёт); осталось единственное звено, требующее аккаунта — регистрация в Developer Console и подача листинга. См. `docs/chrome-web-store.md` и `docs/oss-release.md`.
